<?php
	...
	if($primVez=='true'){
		$ultimoID="select max(id_evento) as nuevoID From evento ";
		...
		$insertar = $conex->consulta("INSERT INTO evento (id_evento, titulo, resumen, descripcion, fecha, hora, fecha_alta, cont_visitas, url_facebook, url_twitter) VALUES(".$ultimoID.",'".$titulo."','".$resumen."','".$descripcion."','".$fecha."','".$hora."',curdate(),0,'".$facebook."','".$twitter."')");
		
		if (!$insertar){
			die("Fallo en la insercion del Evento en la Base de Datos.");
		}
		$idUsuario = $_POST['idUsuario'];
		$insertar = $conex->consulta("INSERT INTO usuario_evento(id_usuario, id_evento) VALUES (".$idUsuario.",".$ultimoID.")");
		
		if (!$insertar){
			die("Fallo en la insercion del usuario - Evento en la Base de Datos: ".mysql_error());
		}
	}else{
			...
			<div class="gallery-section">	   
                ...
                <div >
                	<table>
                    	<tr>
                        	<td>
			                    <h4 class="top" align="center">Aquí podrás agregar las imágenes de tu evento.</h4>
                            </td>
                        </tr>
                        <tr>
                        	<td align="left">
                        ...
                        <tr>
                        	<td align="left">
			                    Agrega TODAS las imágenes de tu evento, empezando por la que lo representa mejor!<br />
						...
						for($i=0;$i<count($_FILES["archivo"]["name"]);$i++)
						{
							...
							if($_FILES["archivo"]["type"][$i]=="image/jpeg" || 
								...
								if(file_exists($carpetaDestino) || @mkdir($carpetaDestino))
								{
									...
									$insertar = $conex->consulta("INSERT INTO foto_evento (id_evento, numero_foto, ubicacion_foto) VALUES (".$ultimoID.",".$cont.",'".$rutaDestino."')");
									if (!$insertar){
										?> <img src="images\imgGenerales\tache.png" height="50" width="50" alt=""> <?php
										die("Fallo en la insercion de la Foto número ".$cont." del Evento en la Base de Datos: ".mysql_error());
									}
								}else{
									?> <img src="images\imgGenerales\tache.png" height="50" width="50" alt=""> <?php
                ...
                <?php
					if($cont > 0){
					$qryGaleria="SELECT evento.id_evento, foto_evento.ubicacion_foto FROM evento LEFT JOIN foto_evento ON evento.id_evento = foto_evento.id_evento WHERE evento.id_evento = $ultimoID";
               ...
               <div class="gallery-section">
                ...
                <h4 class="top"><a href="detalleEvento.php?idEvento=<?php echo $ultimoID ?>" >Ver Mi Evento <span class="glyphicon glyphicon-circle-arrow-right"></span></a></h4>
            	<?php
					}
				?>
			</div>
